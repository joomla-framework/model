...
`getDb()`, `setDb()`, `getState()` and `setState()` have the same signatures in 3.0.0 as in 2.0.0,
and both interfaces are unchanged.
...
The optional packages in `suggest` moved to their 3.x releases: `joomla/database` and
`joomla/registry`.
